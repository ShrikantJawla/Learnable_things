...
const bobEditColdCallingModel = require("../../model/bob/bobEditColdCallingModel");
let controllerObj = {}
/* A function which is used to render the `bobColdCalling.ejs` file. */
controllerObj.renderColdCallingsUi = async function(req, res, next){
	...
	if (middleObj) {
		...
		/* Rendering the `bobColdCalling.ejs` file. */
	} else {
...
controllerObj.renderEditColdCallingsUi = async function(req, res, next){
	...
	if (middleObj) {
		...
		let coldCallingData = await bobEditColdCallingModel.getBobEditColdCallingData({idToFind: req.query.id});
