...
userAdminObj.userAdminListUI = async function (req, res, next) {
  ...
  if (middleObj) {
    ...
    let moduleName = Object.values(uaColumns.allTr[0])[0]
      .replace("edit-", "")
      .split("?id")[0];
...
userAdminObj.addNewAdminUser = async function (req, res, next) {
  console.log("adfffffffffff......", req.body);
  let returnData = {
    status: false,
...
userAdminObj.updateAdminUser = async function (req, res, next) {
  ...
  SET ua_name = '${userName}', ua_email = '${email}', ua_role = ${+role},active_user=${activeStatus}
