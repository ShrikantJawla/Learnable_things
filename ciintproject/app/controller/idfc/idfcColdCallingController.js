...
const idfcEditColdCallingModel = require("../../model/idfc/idfcEditColdCallingModel");

let controllerObj = {}
/* A function which is used to render the `idfcColdCalling.ejs` file. */
controllerObj.renderColdCallingsUi = async function(req, res, next){
	...
	if (middleObj) {
		...
		/* Rendering the `idfcColdCalling.ejs` file. */
	} else {
...
controllerObj.renderEditColdCallingsUi = async function(req, res, next){
	...
	if (middleObj) {
		...
		let coldCallingData = await idfcEditColdCallingModel.getIdfcEditColdCallingData({idToFind: req.query.id});
