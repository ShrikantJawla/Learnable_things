...
let axisTeleApplicationsModel = require('../../model/axis/axisTeleModel')
...
controllerObj.getTeleApplicationsNew = async function (req, res, next) {
    ...
    if (middleObj) {
        ...
        let axisTeleColumns = await axisTeleApplicationsModel.getAxisTeleColumns();
        let displayName = 'AXIS BANK TELE APPLICATION NEW ';
...
controllerObj.teleLeadsUi = async function (req, res, next) {
    ...
    if (middleObj) {
        let sideBarData = await commonController.commonSideBarData(req)
        res.render('axisBank/axisTeleLeads', { sidebarDataByServer: sideBarData })
    } else {
        res.render("error/noPermission")
    }
}
...
controllerObj.getTeleApplicationsAjax = async function (req, res, next) {
    ...
    if (req.body.dropOff) {
        dataFromDb = await axisTeleApplicationsModel.getFilteredAxisApplicationsWithIpaOriginalStatusDistinct(req.body, userdata);

    } else {
        dataFromDb = await axisTeleApplicationsModel.getFilteredAxisApplications(req.body, userdata);
    }
...
controllerObj.editApplicationUi = async (req, res, next) => {
    ...
    if (middleObj) {
        ...
        res.render('axisBank/editTeleApplication', { sidebarDataByServer: sideBarData })
    } else {
        res.render("error/noPermission")
    }
}
